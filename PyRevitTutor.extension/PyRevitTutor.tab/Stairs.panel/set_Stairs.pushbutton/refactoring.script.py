...
def go_through_all_railings(associated_railings, lengthPanel, r_depth, r_height, runs_actualNumberOfTreads,
...
def getting_parameters_of_element(el):
    ...
    return coatingMarble_param, LineForBlindPeople_param, PanelStairs_param, actualNumberOfRises, Volume_param


def getting_parameters_for_runs(el, coatingMarbleResult, r_depth, r_height, blindLineLength):
...
for el in stairs_collector:
    ...
    coatingMarble_param, \
        LineForBlindPeople_param, \
        PanelStairs_param, actualNumberOfRises, \
        Volume_param = getting_parameters_of_element(el)

    # getting material ids and calculating total volume of the stairs
    ...
    for r in runs:
        runs_element, \
            ...
            runs_actualNumberOfRises, \
            runs_actualNumberOfTreads \
            = getting_parameters_for_runs(r, coatingMarbleResult, r_depth, r_height, blindLineLength)
        associated_railings = isRailingAssociatedWithRunCheck(railings, runs_element)
        associated_railings = associated_railings[-2:]
        lengthPanel = go_through_all_railings(associated_railings,
                                              lengthPanel,
                                              r_depth,
                                              r_height,
                                              runs_actualNumberOfTreads,
    ...
    # Beginning of transaction and setting all the parameters for a specific stair in loop
